...
- **`bbox` ต่อ block เป็นทางเลือก — เป็น "ชั้นสำรอง" ของการวาดกรอบแดง**: ระบบหา
  ตำแหน่ง "คำที่มีปัญหา" จาก **① พิกัดคำใน PDF text layer → ② Tesseract** ก่อนเสมอ
  เพราะสองชั้นนั้นเป็นพิกัดที่ *วัด* มาจริง ส่วน `blocks[].bbox` ของ LLM เป็นการ
  *ประมาณ* จึงใช้เป็น **ชั้น ③** และ**ต้องผ่านการอ่านซ้ำพิสูจน์ว่ากรอบตรงคำจริงก่อน
  ถึงจะวาด** (ถ้าพิสูจน์ไม่ได้ = ไม่วาด). ผลตรวจ PASS/FAIL ไม่เปลี่ยนไม่ว่ากรณีใด
  > ⚠️ เดิมชั้นนี้เคยถูกใช้เป็น *ชั้นแรก* และไม่ตรวจสอบเลย → บนงานจริงกรอบไปโผล่
  > คนละแถวในตารางโภชนาการ จึงสลับลำดับและเพิ่มการพิสูจน์ **ห้ามสลับกลับ**
  - รองรับพิกัด 3 แบบอัตโนมัติ: สัดส่วน `0..1`, สเกล `0..1000` (แบบที่ Gemini
    มักคืน) และพิกเซลจริงของภาพที่ส่งไป OCR — ระบบตัดสิน convention **ต่อโซน**
    โดยเทียบกับขนาดภาพจริงที่ส่งไป (เก็บไว้ในฟิลด์ `ocr_wh` ของผล OCR)
  - **ยิ่งแบ่ง block ละเอียดต่อคำ/ต่อบรรทัด ยิ่งชี้ตำแหน่งได้ตรง** (block ที่รวม
    ทั้งย่อหน้าจะได้กรอบกว้างเกินจนไม่ช่วยอะไร ระบบจะข้ามกรอบที่เกือบเท่าทั้งภาพ)
