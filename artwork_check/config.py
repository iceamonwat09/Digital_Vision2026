...
# ── กรอบแดง: บรรทัดที่ "ไหลข้ามแถว" (wrap) ──────────────────────────
# ``_phrase_matches`` ต้องหาคำของวลีให้ครบ **ติดกันในแถวเดียวกัน**. แต่
# backend อย่าง Gemini คืนย่อหน้าที่ถูกจัดหน้าใหม่มาเป็น "หนึ่งบรรทัด"
# ขณะที่ Tesseract อ่านตามแถวจริงบนภาพ ⇒ วลีเดียวกันถูกหั่นเป็น 3 แถว
# ⇒ ไม่มีหน้าต่างไหนครบ ⇒ **0 กรอบเสมอ** ไม่ว่าอ่านถูกแค่ไหน (วัดแล้ว
# บรรทัดไหล 3 แถว = 0 กรอบ · บรรทัดไม่ไหล = 1 กรอบ)
#
# ทางแก้: เมื่อทางเดิม **ล้มเหลวแล้วเท่านั้น** ให้ยิงเป็น "ช่วงคำติดกันที่
# ยาวที่สุด" ของวลีในแต่ละแถว ⇒ ได้กรอบหนึ่งใบต่อแถวที่วลีไหลไป
# (วัดจริง: 0 → 3 กรอบ ตกตรงแถวของวลีพอดี · วลีที่ไม่มีในภาพแต่ใช้คำ
# ร่วมกัน ยังได้ 0 กรอบ = ไม่ฟ้องผิด · เคสที่ทางเดิมสำเร็จไม่ถูกแตะเลย
# เชิงโครงสร้าง เพราะชั้นนี้ทำงานหลังทางเดิมคืนค่าว่าง)
# ตั้ง 0 = กลับพฤติกรรมก่อน 18 ก.ย. 100%
HIGHLIGHT_WRAP_RUNS = os.getenv(
    "ARTWORK_HIGHLIGHT_WRAP_RUNS", "1").strip().lower() not in ("0", "false", "")
# ด่านกันชี้ผิด: ช่วงหนึ่งต้องยาวพอจะ "เฉพาะเจาะจง" จริง ๆ —
# ≥ 2 คำ **และ** ≥ 8 ตัวอักษร. คำเดียวโดด ๆ ที่บังเอิญเป็นส่วนหนึ่งของวลี
# (เช่น "Calcium" ในตารางโภชนาการ ขณะที่วลีคือ "D-Calcium Pantothenate…")
# จะไม่ผ่านด่านนี้ ⇒ ไม่ถูกวาด
HIGHLIGHT_RUN_MIN_WORDS = int(
    os.getenv("ARTWORK_HIGHLIGHT_RUN_MIN_WORDS", "2"))
HIGHLIGHT_RUN_MIN_CHARS = int(
    os.getenv("ARTWORK_HIGHLIGHT_RUN_MIN_CHARS", "8"))
# ── บอกเหตุผลเมื่อ "ไม่มีกรอบแดง" (advisory ล้วน) ────────────────────
# ผู้ใช้เห็นว่า "บางครั้งมี บางครั้งไม่มี" แล้วไม่มีอะไรบอกว่าทำไม. เหตุผล
# บางข้อรู้ได้ตั้งแต่ตอนส่งตรวจโดย **ไม่ต้องอ่านภาพเลย** คือ
#   ① เครื่องนี้ไม่มี Tesseract       ⇒ ชั้นวัดพิกเซลใช้ไม่ได้ทั้งชั้น
#   ② ไม่มี traineddata ของสคริปต์นั้น ⇒ คำอาหรับ/ไทย/จีนไม่เคยได้กรอบ
# (ข้อ ② เกิดจริงบนสถานีมาแล้ว — ลง tesseract แบบ eng อย่างเดียว แล้วคำ
# อาหรับไม่มีกรอบเลย โดยไม่มีอะไรบอก). ทั้งสองข้อ **ผู้ใช้แก้ได้เอง**
# จึงคุ้มที่จะบอก · เหตุผลที่รู้ได้เฉพาะตอนอ่านภาพ (หาคำไม่เจอ) **ไม่เดา**
# ตั้ง 0 = ไม่แสดงเหตุผลเลย = พฤติกรรมเดิม
HIGHLIGHT_WHY = os.getenv(
    "ARTWORK_HIGHLIGHT_WHY", "1").strip().lower() not in ("0", "false", "")
...
# ── รายงานต้องแสดงภาพ "แนวเดียวกับที่ผู้ใช้เพิ่งจัดมา" ────────────────
#
# ที่มา (ผู้ใช้ 11 ก.ย.): ปุ่ม "↻ หมุนจอ" ทำให้ลากโซนบนฉลากที่ตัวหนังสือ
# ตั้งทั้งใบได้แล้ว **แต่พอกดส่งตรวจ รายงานเหวี่ยงกลับไปแนวเดิม**
# ⇒ ผู้ตรวจต้องเอียงคอดูเองทั้งที่เพิ่งจัดให้ตรงมาแล้ว
#
# ครอบ 2 อย่าง ซึ่งเป็นเรื่องเดียวกันคนละระดับ:
#   ① **ภาพทั้งหน้า** (``preview.png``/``overlay.png``) หมุนตามมุมที่จอ
#      หมุนอยู่ตอนลากโซน (``report.json: page_rot``)
#   ② **ภาพในการ์ด defect** หมุนตามมุมที่ผู้ใช้ปักหมุดไว้ที่โซนนั้น
#      (``report.json: zones[].view_rot``) — เดิมพังเงียบ: ``read_zone``
#      คืน ``rotate: 0`` ตายตัวในเส้นทาง ``pdf-text``/``none`` (ไม่ได้หมุน
#      ภาพจริง เพราะอ่านจาก text layer) แล้ว ``run_inspection`` เอาค่านั้น
#      ไป **ทับมุมที่ผู้ใช้ปักหมุด** ⇒ ไฟล์ PDF ที่มี text layer (เคสที่พบ
#      บ่อยที่สุด) การ์ดไม่เคยหมุนตามเลยไม่ว่าจะตั้งมุมไว้แค่ไหน
#
# ⚠️ **แสดงผลล้วน** — หมุนตอน *เสิร์ฟ* ภาพเท่านั้น:
#    ไฟล์บนดิสก์ไม่ถูกแตะ · พิกัดโซนที่เก็บยังเป็นของหน้าที่ไม่หมุนเสมอ ·
#    ``rotate`` (มุมที่ OCR หมุนจริง) **ไม่ถูกแก้** ⇒ ชั้น pixel / overlay /
#    highlight / propose_zones / snap_bbox / autopair เห็นค่าเดิมทุกตัว
#
# ตั้ง ``0`` = ไม่หมุนอะไรในรายงานเลย = พฤติกรรมก่อน 11 ก.ย. 100%
...
#
# ที่มา (สถานี 26 ก.ย., ฉลากซองที่ตัวหนังสือตั้งทั้งใบ): ผู้ใช้หมุนจอ 90°
# แล้วลากโซน 🅰 (ได้ rotate=90 ถูกต้อง) → กด "หากรอบคู่อัตโนมัติ" ⇒ โซน 🅱
# ถูกสร้างด้วย ``rotate: "default"`` ตายตัว ⇒ OCR อ่านฝั่ง 🅱 แบบตะแคง
# แล้ว **ตกท่อน ``........ (min.) 8%`` ไปทั้งท่อน** ⇒ ขึ้นการ์ด
# "พบเฉพาะใน โซน 2" ซึ่งเป็น **การ์ดปลอมใบเดียวของรายงาน** (ภาพพิมพ์ครบ
# ทั้งสองไฟล์) · การ์ดฝั่ง 🅱 ก็ไม่หมุนตามด้วยเหตุเดียวกัน
#
# * หากรอบคู่อัตโนมัติ ⇒ โซน 🅱 ได้ ``rotate`` เดียวกับโซน 🅰 ต้นทาง —
#   ``zones.autopair_bbox`` ค้นแบบไม่หมุน (matchTemplate แกนตรง) ⇒ จับคู่
#   ติดได้ก็ต่อเมื่อบล็อกบน 🅱 **วางแนวเดียวกับ 🅰** อยู่แล้ว ⇒ มุมเดียวกัน
#   ถูกต้องเชิงโครงสร้าง (และเข้าหลัก "สองฝั่งอ่านแบบเดียวกัน")
# * "เสนอโซนใหม่" ขณะหมุนจอ ⇒ โซนที่เสนอได้มุมของจอ เหมือนโซนที่ลากเอง
#
# ตั้ง ``0`` = โซนที่ระบบสร้างได้ ``"default"`` เหมือนก่อน 26 ก.ย. เป๊ะ
ZONE_ROTATE_INHERIT = os.getenv(
    "ARTWORK_ZONE_ROTATE_INHERIT", "1").strip().lower() not in ("0", "false", "")
