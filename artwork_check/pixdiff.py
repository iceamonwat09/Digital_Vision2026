...
_REASON_TEXT = {
    ...
    "panel_too_different":
        "ทาบภาพติดแล้ว แต่เนื้อหาต่างกันมากเกินกว่าจะชี้เป็นจุด ๆ ได้ "
        "(แผงเดียวกันที่แก้ข้อความยกบล็อกยังต่างไม่ถึง 20% — เกินกว่านั้น "
        "มักแปลว่าจับคู่คนละแผง) ⇒ ไม่ชี้จุดดีกว่าชี้ผิดที่ · "
        "ดูภาพผลตรวจสองฝั่งเทียบด้วยตา แล้วถ้าเป็นแผงเดียวกันจริง "
        "ให้ลากโซนให้แคบลงทีละบล็อกแล้วส่งใหม่",
    "scale_out_of_range":
        "แผงในสองไฟล์ขนาดจริงต่างกันมากเกินกว่าจะปรับให้เท่ากันได้ "
        "(เช่นไฟล์หนึ่งเป็นปรู๊ฟย่อ) — ระบบเพิ่มความละเอียดของฝั่งที่เล็ก "
        "ให้แล้วแต่ยังไม่พอ ⇒ ไม่เทียบดีกว่าชี้ผิดที่",
    "zone_too_different":
    ...
    "edge_only":
        "ความต่างที่พบอยู่ติดขอบโซนทั้งหมด = เนื้อหารอบแผงที่ลากเกินเข้ามา "
        "(สองไฟล์วางแผงคนละที่ ของรอบ ๆ จึงคนละอย่าง) — แยกไม่ออกว่าของจริง "
        "ปนอยู่ในนั้นไหม จึงไม่ตัดสิน ลากโซนให้ครอบเฉพาะแผงแล้วลองใหม่",
}
...
def compare_images(img_a, img_b,
                   ...
                   max_diff_ratio: float = MAX_DIFF_RATIO) -> dict:
    ...
    # ``max_diff_ratio`` แยกออกมาเป็นพารามิเตอร์เพราะ **โหมดโซนกับโหมดทั้งหน้า
    # ต้องใช้เพดานคนละค่า** — โหมดโซนวัดได้ว่าแผงเดียวกันที่แก้ข้อความยกบล็อก
    # ยังไปถึง 19.96% ซึ่งชิดเพดาน 20% เหลือระยะแค่ 0.04 จุด
    # ค่าเริ่มต้น = ค่าเดิม ⇒ ผู้เรียกที่ไม่ส่งมาได้พฤติกรรมเดิมเป๊ะ
