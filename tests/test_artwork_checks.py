...
# ── Phase 1: single-char noise lines dropped from panel voting ─────────

def test_voting_drops_single_char_noise_line():
    """อักษรเดี่ยวที่ OCR จับจากโลโก้/กราฟิก (C/ﺫ/כ) ต้องไม่ฟ้อง MISMATCH."""
    panel = "HIDDEN BAY\nSKIPJACK TUNA\nNET WEIGHT 170g"
    for stray in ("C", "ﺫ", "כ", "1", "الله"[0]):
        zones = [dict(_zone("z1"), doc="a"), dict(_zone("b1"), doc="b")]
        defects = check_group_consistency(
            zones, {"z1": panel + "\n" + stray, "b1": panel})
        assert [d for d in defects
                if d["class"] == "MISMATCH_PANELS"] == [], stray


def test_voting_single_char_filter_keeps_real_typo():
    """ตัวกรองอักษรเดี่ยวต้องไม่กลบ typo จริง (บรรทัดยาว >1 ตัว)."""
    zones = [_zone("z1"), _zone("z2"), _zone("z3")]
    ok = "HIDDEN BAY\nSKIPJACK TUNA"
    defects = check_group_consistency(
        zones, {"z1": ok, "z2": ok, "z3": "HIDDEN BAY\nSKIPJAK TUNA"})
    assert any(d["class"] == "MISMATCH_PANELS" for d in defects)


# ── Phase 2: re-verify on mismatch (stability across 2 OCR passes) ─────

def _run_reverify(monkeypatch, tmp_path, a_passes, b_passes, enabled=True):
    np = pytest.importorskip("numpy"); cv2 = pytest.importorskip("cv2")
    import os
    from artwork_check import pipeline, report, config as awcfg
    monkeypatch.setattr(report.config, "INSPECTIONS_DIR", str(tmp_path))
    monkeypatch.setattr(awcfg, "STABILITY_REVERIFY", enabled)
    d = report.inspection_dir("20260101-000000-abcd33", create=True)
    for f in ("source.png", "source_b.png"):
        open(os.path.join(d, f), "wb").write(b"x")
    for f in ("preview.png", "preview_b.png"):
        cv2.imwrite(os.path.join(d, f), np.full((40, 60, 3), 255, np.uint8))
    monkeypatch.setattr(pipeline, "ArtworkDocument",
                        lambda *a, **k: type("D", (), {
                            "render": lambda s, dpi: np.full((40, 60, 3),
                                                             255, np.uint8)})())
    cnt = {"z1": 0, "b1": 0}
    ...
    def fake_read(doc, zones, page_auto=False):
        out = []
        for z in zones:
            k = "z1" if z["id"] == "z1" else "b1"
            passes = a_passes if k == "z1" else b_passes
            out.append({"zone_id": z["id"], "text": passes[min(cnt[k], 1)],
                        "engine": "stub", "conf": 0.96, "rotate": 0})
            cnt[k] += 1
        return out
    monkeypatch.setattr(pipeline.ocr, "read_all_zones", fake_read)
    zones = [{"id": "z1", "type": "panel", "group": "A",
              "bbox": [.1, .1, .2, .2], "doc": "a"},
             {"id": "b1", "type": "panel", "group": "A",
              "bbox": [.1, .1, .2, .2], "doc": "b"}]
    rep = pipeline.run_inspection("20260101-000000-abcd33", zones)
    return rep, cnt


_RV_BODY = ("Ingredients: Tuna, Salt.\n"
            "Manufacturing Co., Ltd, Thailand.\nSEE ON CAN")


def test_reverify_suppresses_ocr_noise_mismatch(monkeypatch, tmp_path):
    """เคสจริง: doc a อ่าน STEEL ทั้ง 2 รอบ, doc b อ่านแค่รอบเดียว →
    flaky → verdict สะอาด (0 MISMATCH) และ re-OCR ทำงาน (2 passes)."""
    a = [_RV_BODY + "\nSTEEL\nNET 1'\nWEIGHT\n170g",
         _RV_BODY + "\nSTEEL\nNET\nWEIGHT\n170g"]
    b = [_RV_BODY + "\nNET\nWEIGHT\n170g",
         _RV_BODY + "\nSTEEL\nNET\nWEIGHT\n170g"]
    rep, cnt = _run_reverify(monkeypatch, tmp_path, a, b)
    assert [d for d in rep["defects"]
            if d["class"].startswith("MISMATCH")] == []
    assert cnt["z1"] == 2                         # ตรวจซ้ำจริง
    # report คง OCR ดิบไว้แสดง
    assert "STEEL" in next(r["text"] for r in rep["ocr"]
                           if r["zone_id"] == "z1")


def test_reverify_keeps_real_stable_difference(monkeypatch, tmp_path):
    """ความต่างจริงที่ทั้งสองฝั่งอ่านนิ่ง (Sodium 24% vs 20%) ต้องยังฟ้อง."""
    a = [_RV_BODY + "\nSodium 475mg 24%\nNET\n170g"] * 2
    b = [_RV_BODY + "\nSodium 475mg 20%\nNET\n170g"] * 2
    rep, _ = _run_reverify(monkeypatch, tmp_path, a, b)
    mm = [d for d in rep["defects"] if d["class"] == "MISMATCH_PANELS"]
    assert mm and "24" in mm[0]["found"]


def test_reverify_disabled_is_single_pass(monkeypatch, tmp_path):
    """ปิด flag → พฤติกรรมเดิม: OCR รอบเดียว + ฟ้องตามเดิม."""
    a = [_RV_BODY + "\nSTEEL\nNET 1'\n170g",
         _RV_BODY + "\nSTEEL\nNET\n170g"]
    b = [_RV_BODY + "\nNET\n170g"] * 2
    rep, cnt = _run_reverify(monkeypatch, tmp_path, a, b, enabled=False)
    assert cnt["z1"] == 1                         # ไม่ตรวจซ้ำ
    assert [d for d in rep["defects"]
            if d["class"] == "MISMATCH_PANELS"]   # ยังฟ้อง (เดิม)


def test_reverify_no_mismatch_no_extra_ocr(monkeypatch, tmp_path):
    """ไม่มี mismatch → re-verify ไม่ยิง OCR ซ้ำ (ต้นทุน 0)."""
    same = [_RV_BODY + "\nNET\n170g"] * 2
    rep, cnt = _run_reverify(monkeypatch, tmp_path, same, same)
    assert cnt["z1"] == 1


# ── snap pad (auto-snap on draw, minimal movement) ────────────────────

def test_snap_bbox_accepts_pad():
    np = pytest.importorskip("numpy"); cv2 = pytest.importorskip("cv2")
    from artwork_check.zones import snap_bbox
    img = np.full((400, 600, 3), 255, np.uint8)
    cv2.putText(img, "HELLO", (180, 220), cv2.FONT_HERSHEY_SIMPLEX,
                2, (0, 0, 0), 4)
    drawn = [0.20, 0.35, 0.55, 0.30]
    tight = snap_bbox(img, drawn, pad=0.02)
    assert tight[2] < drawn[2] and tight[3] < drawn[3]   # หดเข้าหาเนื้อหา
