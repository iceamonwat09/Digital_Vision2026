...
> ▶️ **เริ่มรอบหน้าตรงนี้: ให้ผู้ใช้ทดสอบโหมดใหม่กับกล้องจริงบนสถานี**
> ```
...
- Tests: `pytest tests/` — **609 ผ่าน / 5 fail (pre-existing)** (artwork/label/barcode/auth —
  **ตอนนี้ครอบคลุมกล้อง Hikrobot แล้วผ่าน SDK ปลอม แต่ยังไม่ครอบคลุม live loop ของ USB/RTSP**).
  เพิ่มล่าสุด: `tests/test_hik_camera.py` **25 ตัว** + `tests/test_hik_routes.py` **10 ตัว** +
