...
**Branch: `claude/hikrobot-camera-modes-s2hxiy`** (ห้าม push main)
...
| 6 | เทสต์ **74 ตัว** + เอกสาร | ✅ เสร็จ (**ยังไม่ได้ทดสอบกับกล้องจริง**) |
...
### ⚡ แผง "ปรับแสงด่วน" — เกน/เวลารับแสง + ตัวเลขความสว่างจริง (20 ส.ค. 2026)

ผู้ใช้แจ้งว่า **ภาพมืดที่ exposure 2635 µs** ⇒ ยก 2 ค่าที่ต้องจูนบ่อยที่สุดออกมาไว้
**นอกกล่องตั้งค่า** (ไม่ต้องกดกาง) เป็นสไลเดอร์ปรับสด:

| ส่วน | รายละเอียด |
|---|---|
| สไลเดอร์ **เกน (Gain)** | ช่วงมาจากกล้องจริง (สถานีนี้ 0–23.98 dB) · ตั้งได้ระหว่างสตรีม ไม่หยุดภาพ |
| สไลเดอร์ **เวลารับแสง (Exposure)** | **สเกล log** (ช่วงจริง 15 µs–10 วินาที) เพราะสเกลเชิงเส้นจะใช้ไม่ได้เลย · เพดานสไลเดอร์ 100 ms ค่าที่ยาวกว่านั้นใช้ช่องตัวเลขในกล่องตั้งค่า |
| ป้าย **ความสว่างที่วัดได้** | `สว่าง 67/255 · ล้น 0.2%` — สีส้ม = มืด · แดง = พิกเซลล้น · เขียว = ใช้งานได้ · **หยุดสตรีมแล้วล้างเป็น "—"** (ไม่ค้างค่าเก่าให้เข้าใจผิดว่ายังวัดอยู่) |
| คำแนะนำเชิงตัวเลข | คำนวณย้อนจากฟิสิกส์: สว่าง ∝ เวลารับแสง และ ∝ `10^(dB/20)` ⇒ บอกตรง ๆ ว่า **"ต้องการอีกราว X dB จึงจะได้ ~80/255"** |

**คำแนะนำแยก 3 สถานการณ์ (ไม่ใช่พ่นตัวเลขอย่างเดียว):**
1. `mean < 1` (มืดเกือบสนิท) → **ไม่คำนวณ dB** เพราะหารด้วยเลขใกล้ศูนย์ = ตัวเลขที่ดูแม่นแต่ไร้ความหมาย
   ⇒ บอกให้ "เปิดรูรับแสง/เพิ่มไฟ/เพิ่มเวลารับแสงก่อน"
2. ต้องการ dB **มากกว่าเกนที่เหลือ** → "เกนเหลืออีกแค่ Y dB — ต้องเพิ่มไฟหรือเปิดรูรับแสง เกนอย่างเดียวไม่พอ"
3. ต้องการ dB พอไหว → เตือนว่า **เกนสูง = noise กลืนรอยบุบตื้น** และถ้ากระป๋องนิ่งให้เพิ่มเวลารับแสงแทน

**การส่งค่า:** ขณะลากส่งแบบหน่วง 150 ms (`save:false`) · ปล่อยเมาส์จึงส่งค่าสุดท้าย
พร้อมบันทึกลงไฟล์ (`save:true`) — ไม่งั้นจะยิงหลายสิบคำขอ/วินาทีและเขียนไฟล์ค่าตั้งรัว ๆ.
ค่าที่ตั้งได้จะถูก **sync กลับเข้าช่องตัวเลขในกล่องตั้งค่า** เสมอ (ไม่ให้สองที่แสดงคนละค่า)

**`config.HIK_DEFAULTS["gain_db"]` เปลี่ยนจาก 0 → 12 dB** = ขยาย 4 เท่า ⇒ จากที่วัดได้
20/255 ที่ 5000 µs จะได้ราว 80/255. ⚠️ **เกนไม่ได้เพิ่มแสง** — มันขยายทั้งสัญญาณและ noise
(ค่านี้เป็นแค่จุดตั้งต้นให้เห็นภาพ ไม่ใช่คำตอบของเรื่องไฟตาม §6 ของแผน)

> 🧪 **SDK ปลอมจำลองความสัมพันธ์ของแสงแล้ว**: ความสว่าง ∝ exposure และ ∝ `10^(dB/20)`
> โดยตั้งจุดอ้างอิงที่ **9/255 ที่ 2635 µs gain 0** = ตัวเลขที่วัดได้จริงบนสถานี ⇒
> เทสต์ `test_gain_change_brightens_the_image` พิสูจน์ว่า **ปรับเกนแล้วภาพสว่างขึ้นจริง**
> ไม่ใช่แค่ค่าบนจอเปลี่ยน

**ปุ่ม/ค่าที่มีในแผง:** เลือกกล้อง (ตามซีเรียล) · **สไลเดอร์เกน/เวลารับแสง + ความสว่างที่วัดได้** ·
ROI สำเร็จรูป (เต็ม/ครึ่งกลาง/¼ กลาง) ·
...
- ✅ บน container: **74 เทสต์ผ่าน** + รัน `app.py` ตัวจริงกับ **SDK ปลอม** แล้วขับด้วย
  **Chromium จริง** — แท็บสลับถูก, สร้างตัวควบคุม 19 ตัวจากช่วงจริงของกล้อง,
  ...
  ลากสไลเดอร์เกน 0→18 dB แล้ว **ความสว่างที่วัดได้ขึ้นจาก 67 → 135/255 จริง**
...
> ## ▶️ เริ่มรอบหน้าตรงนี้ (สถานะ ณ 20 ส.ค. 2026)
>
> **โค้ดพร้อมหมดแล้ว — เหลือแค่ทดสอบกับกล้อง/ไลน์จริง**
> ```
> git pull            # branch claude/hikrobot-camera-modes-s2hxiy
> py -3.9 app.py      # footer ต้องขึ้น 2026.08.19-hikrobot
> ```
> หน้าตรวจสด → แท็บ **"กล้องอุตสาหกรรม"** → 🔄 ค้นหา → **Start** (⚠️ ปิด MVS ก่อน)
>
> **① จูนแสงด้วยแผง "⚡ ปรับแสงด่วน"** (อยู่บนสุดของแท็บ ไม่ต้องกดกางอะไร)
> เลื่อนเกน/เวลารับแสงจนป้ายความสว่างเป็น **สีเขียว (~80/255)** และ **ล้น < 1%**
> — คำแนะนำใต้สไลเดอร์จะบอกเองว่าต้องเพิ่มอีกกี่ dB
>
> **② สิ่งที่ต้องเก็บกลับมา**
> - fps จริง · **เฟรมหาย/แพ็กเก็ตหาย = 0 ไหม** (แถบสถิติมุมซ้ายบนของภาพ)
> - ค่าเกน/เวลารับแสงที่จูนแล้วได้ภาพดี ⇒ เอาไปตั้งเป็น `HIK_DEFAULTS` ตัวจริง
> - **ผลตรวจของ `bestX.pt` บนภาพกล้องใหม่** (คาดว่าจะไม่ดีเพราะ domain shift —
>   ถ้าเป็นแบบนั้นคือเรื่องปกติ ให้เก็บ dataset ไปเทรน ไม่ใช่ไปแก้โค้ด)
> - ปุ่ม "ถ่าย 1 เฟรม" (imgsz 1280 บนภาพเต็ม 5MP) ให้ผลต่างจากภาพ live ไหม
>
> **③ ถ้าจะไปทดสอบกับไลน์จริง** — อ่านหัวข้อ "🏭 ทดสอบกับไลน์ 7 ใบ/วินาที" ด้านล่างก่อน
> (สรุป: อย่าเชื่อตัวนับบนหน้าเว็บ · ให้ติ๊ก "เก็บภาพชุดข้อมูล" ตอนไลน์วิ่ง แล้ววัดผลด้วย
> `hik_offline_eval.py --fps 69 --pitch-mm <ระยะห่างใบ> --exposure-us <ที่ตั้งไว้>`)
>
> **④ งานที่ยังไม่ได้ทำ (ตามลำดับความสำคัญ)**
> 1. เทรนโมเดลใหม่ด้วยภาพจากกล้อง/ไฟชุดใหม่ (§9 ของแผน) — ตัวชี้ขาดความแม่น
> 2. ไฟ LED ให้ใช้ exposure 150-300 µs ได้ (§6) — ตัวชี้ขาดว่าจะขึ้นไลน์จริงได้ไหม
> 3. triggered mode จาก photo-eye (§4) — ทำเมื่อข้อ 1-2 ผ่านแล้ว

**⚠️ บทเรียน UI ที่เพิ่งเสียเวลาไป:** `.camera-panel` ที่ทุกแท็บใช้ร่วมกันเป็น
**flex แนวนอน** (`display:flex; align-items:center` ไม่มี `flex-direction`) — แท็บเดิม
มีลูก 1-2 ตัวจึงไม่เคยเห็นปัญหา แต่แท็บใหม่มีลูก 12 ตัว ⇒ ถูกจัดเรียง **ข้างกัน**
ในคอลัมน์กว้าง 263px แล้ว **ตัวหนังสือแตกเป็นแนวตั้งทีละอักษร**.
แก้ด้วย `#panelHik { flex-direction: column }` (กฎเดียวกับที่ `#panelUsb` มีอยู่แล้ว
พร้อมคอมเมนต์เตือนไว้ — แต่ผมมองข้าม). **ห้ามแก้ `.camera-panel` กลาง** เพราะกระทบ
3 แท็บเดิม — มีเทสต์ล็อกทั้งสองข้อ + เทสต์ว่าคอลัมน์ความกว้างตายตัวรวมกันต้องไม่เกิน 200px
...
- Tests: `pytest tests/` — **581 ผ่าน / 5 fail (pre-existing) / 21 skipped** (artwork/label/barcode/auth —
  **ตอนนี้ครอบคลุมกล้อง Hikrobot แล้วผ่าน SDK ปลอม แต่ยังไม่ครอบคลุม live loop ของ USB/RTSP**).
  เพิ่มล่าสุด: `tests/test_hik_camera.py` **37 ตัว** + `tests/test_hik_routes.py` **10 ตัว** +
  `tests/test_hik_offline_eval.py` **11 ตัว** +
...
- CONFIG_VERSION ปัจจุบัน: **`2026.08.19-hikrobot`** (เช็คที่ footer ว่ารันโค้ดใหม่จริง).
