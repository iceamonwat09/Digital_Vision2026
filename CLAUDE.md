...
|---|---|---|
| 1 | **`diagnose_hikrobot.py`** + `tests/fake_mvs/` + `tests/test_hikrobot_diagnose.py` (14 ตัว) | ✅ **เสร็จ + รันบนสถานีแล้ว** |
| 2-6 | `hik_camera.py` → `config.py` → `app.py` → แท็บใน `index.html` → เทสต์+เอกสาร | ⏳ พร้อมเริ่ม |
...
> ▶️ **เริ่มรอบหน้าตรงนี้:** ขั้นที่ 1 ปิดจบแล้ว (เครือข่ายยืนยันเขียวทั้งหมด) —
> **เริ่มขั้นที่ 2 `hik_camera.py` ได้เลย** ค่าที่ต้องใช้ครบแล้วทุกตัว.
> สิ่งที่คลาสใหม่ต้องทำตั้งแต่เปิดกล้อง: ตั้ง **packet 8164 + delay 0** ·
> ใช้ **BayerRG8** แล้วแปลงเป็น BGR ด้วยตัวแปลงของ SDK · ตั้ง **ROI** ·
> ย่อภาพก่อนส่งเข้า pipeline · อ่านตัวตนกล้องจาก **node** ไม่ใช่ struct
...
- Tests: `pytest tests/` — **602 ผ่าน / 5 fail (pre-existing)** (artwork/label/barcode/auth —
  **ไม่ครอบคลุม camera/live loop**).
  เพิ่มล่าสุด: `tests/test_artwork_pixdiff.py` 45 ตัว (pixel diff ทั้งหน้า/โซน + ด่านปฏิเสธ),
...
- CONFIG_VERSION ปัจจุบัน: **`2026.08.19-pixdiff`** (เช็คที่ footer ว่ารันโค้ดใหม่จริง).
