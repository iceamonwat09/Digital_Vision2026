...
- **⚠️ Deploy IIS (Windows Server) — มีคู่มือแล้ว: `docs/DEPLOY_IIS_WINDOWS_SERVER.md`**
  ...
  - **เซิร์ฟเวอร์ใช้ Python 3.9.13 เท่ากับสถานี (การตัดสินใจของเจ้าของระบบ)** —
    ...
    รื้อ environment ใหม่. **ข้อแลกที่บันทึกไว้**: 3.9 หมดการสนับสนุน 31 ต.ค. 2025
    และ **3.9.13 = ตัวติดตั้ง .exe ตัวสุดท้ายบน Windows** (3.9.14+ เป็น source-only)
    → มีมาตรการชดเชยใน **ภาคผนวก G** ของคู่มือ. **ห้าม 3.13** (`numpy 1.26`/
    `Pillow 10.1` ไม่มีล้อ cp313). ถ้าอนาคตยืนยันว่าไม่ต้องใช้กล้อง ย้ายไป 3.11/3.12
    ได้ทันที — **ตรวจแล้วว่าโค้ดไม่ได้ติดอะไร**: ซอร์สทั้งหมด compile ผ่านบน 3.11/3.12
    และไม่มี `distutils`/`imp`/`utcnow()`/`getdefaultlocale()` (ติดแค่ตัวเร่งเท่านั้น).
  - **📷 ถ้าจะเปิดโหมดกล้องบนเซิร์ฟเวอร์ — ดูภาคผนวก F.** จุดที่ต้องรู้: `bestX.pt`
    **ไม่ได้อยู่ในกิต** (`.gitignore` กัน `weights/` + `*.pt`; มีแต่ `best.pt` ที่ถูก
    track ไว้ก่อนกฎ) ต้องคัดลอกมาเอง · ต้องผ่าน `verify_onnx.py`/`verify_openvino.py`
    ก่อนเปิดตัวเร่ง · ตั้ง `VISIONIQ_IIS_INIT=full` · **ยังไม่ได้พิสูจน์ 3 เรื่อง**:
    MJPEG ผ่าน IIS proxy อาจถูก buffer, App Pool (session 0) อาจเข้าถึงกล้อง USB ไม่ได้,
    และ `camera`/`detector` เป็น global ระดับโปรเซส = รองรับครั้งละ 1 session.
    **ทางที่ปลอดภัยกว่า**: ให้ IIS ดูแลแค่ Login+Artwork แล้วรันโหมดกล้องแยกเป็น
    `py -3.9 app.py` บนเครื่องที่ต่อกล้องจริง.
