...
### ▶️ ทำข้อนี้ก่อนอย่างอื่นทั้งหมด — **iGPU ไม่ถูกใช้อยู่**

บนสถานี 25 ส.ค. แถบ perf ขึ้น `... / inf 373ms / **ONNX 🐢**` ⇒ ระบบวิ่งอยู่บน
**ONNX/CPU** ไม่ใช่ iGPU (ตรงกับตัวเลขที่บันทึกไว้: ONNX CPU ≈ 280 ms · iGPU 45-50 ms)

```
git pull
py -3.9 diagnose_accel.py          # ← ตอบว่า "ทำไมไม่ใช้ iGPU" ด้วยหลักฐาน
```
สคริปต์จะพิมพ์ 5 ส่วน (ค่าตั้ง · แพ็กเกจจริง · ไฟล์ export · backend ที่ได้จริง
+ ms/เฟรม · สรุป + วิธีแก้) **ของทุก `.pt` ในโฟลเดอร์** แล้วบอก **ต้นเหตุที่ตรงตัว**

### ✅ ผลรันบนสถานี 25 ส.ค. — iGPU **ทำงานอยู่จริง** (แต่กับ `best.pt`)

```
openvino 2024.6.0 · available_devices ['CPU', 'GPU'] · GPU = Intel(R) Iris(R) Xe Graphics (iGPU)
best.pt → OpenVINO@intel:gpu · inf 24 ms @480 · 86 ms @1280   ✅
```
⇒ **ตัดต้นเหตุ ①②③ ออกหมด** — แพ็กเกจถูกรุ่น ไดรเวอร์เห็น GPU เส้นทางโหลดสำเร็จ

> 🐛 **แต่รันนั้นให้ไฟเขียวผิดตัว — บั๊กในเครื่องมือของผมเอง (แก้แล้ว):**
> สคริปต์ default ไปที่ `config.MODEL_PATH` = **`best.pt`** (detect · คลาส
> `dented/good`) แต่ระบบจริงวิ่ง **`bestX.pt`** (segmentation · คลาส `dent/can`)
> ซึ่งเลือกจากหน้าเว็บ. **แต่ละ `.pt` มี export แยกของตัวเอง** ⇒ ตัวหนึ่งอยู่บน
> iGPU ขณะที่อีกตัวตกไป CPU ได้สบาย ๆ. ตอนนี้ **ตรวจทุก `.pt` ในโฟลเดอร์**
> และพิมพ์ **คลาสของโมเดล** ให้เห็นว่ากำลังวัดตัวไหน (เทสต์ล็อกไว้ 2 ตัว)
> — ตรงกฎเหล็กข้อ 2 เป๊ะ: เครื่องมือวัดที่วัดผิดทางแย่กว่าไม่มีเครื่องมือ

**`inf 373 ms` แปลว่าอะไรได้บ้าง (ทั้งสามอันหน้าตาเหมือนกันบนจอ แต่แก้คนละวิธี):**

| # | ต้นเหตุ | หลักฐานที่สคริปต์พิมพ์ | วิธีแก้ |
|---|---|---|---|
| ① | **ไม่ได้ติดตั้ง `openvino`** (หายไปตอนลง/ถอนแพ็กเกจอื่น) | `openvino ❌ import ไม่ได้` | `py -3.9 -m pip install "openvino==2024.6.0"` |
| ② | ติดตั้งแล้ว **แต่ไม่เห็น GPU** | `available_devices ['CPU']` | ไดรเวอร์ Intel Graphics / BIOS — **ไม่ใช่เรื่องแพ็กเกจ** |
| ③ | เห็น GPU **แต่โหลด/smoke test ล้ม** | `⚠️ OpenVINO ใช้ไม่ได้ (…)` | รุ่น openvino ผิด (ต้อง 2024.6.0 บน py3.9) · ลบ `*_openvino_model` แล้ว export ใหม่ |

> **ฟังก์ชัน iGPU ทำงานได้จริง** — ยืนยันบนสถานีนี้แล้วเมื่อ 2 ก.ค. 2026:
> `verify_openvino.py` **PASS ทั้ง `intel:cpu` และ `intel:gpu` ที่ imgsz 480+1280**
> (IoU 0.9809-0.9913 · Δconf ≤0.0053) และวัดได้ **45-50 ms/เฟรม** ในระบบจริง.
> โค้ดเส้นทางนี้ยังอยู่ครบและ `OPENVINO_DEVICE = "intel:gpu"` ยังตั้งอยู่ ⇒
> **สิ่งที่เปลี่ยนคือสภาพเครื่อง ไม่ใช่ฟีเจอร์** — ตัว `diagnose_accel.py` มีไว้ชี้ว่าเปลี่ยนตรงไหน
