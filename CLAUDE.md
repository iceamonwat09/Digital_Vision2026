...
|---|---|---|
| packet size | **1500** (ควรเป็น **8164**) + delay **400** | ❌ ยังไม่เปิด Jumbo Frame ⇒ **เฟรมหาย 2-3/60 จริง** และ fps ตกเหลือ 15-17 |
| **ROI ครึ่งกลาง 1224×1024** | **66.7 fps · เฟรมหาย 0** | ✅ **คันโยกหลักของโหมดใหม่** — ที่ 12 ใบ/วิ ได้ ~5.5 เฟรม/ใบ (เท่ากล้อง CS016 ที่แผนเลือกไว้) และตรงกับ §7 ที่อยากให้ฝาเต็มเฟรม |
...
> ▶️ **เริ่มรอบหน้าตรงนี้:** (ก) ให้ผู้ใช้เปิด **Jumbo Packet = 9014** ที่ NIC `172.32.1.9`
> แล้วรัน `py -3.9 diagnose_hikrobot.py --set-packet --fps-test` ยืนยันว่าเฟรมหาย = 0
> (ข) เริ่มขั้นที่ 2 `hik_camera.py` ได้เลย — ค่าที่ต้องใช้ครบแล้ว

**⚠️ บั๊กที่เจอจากการรันจริงและแก้แล้ว:** รอบแรกรายงานขึ้น `[0] ? SN=? (?)` เพราะอ่าน
`MV_CC_DEVICE_INFO.SpecialInfo` ของ SDK รุ่นบนสถานีไม่ได้ **แล้วกลืน exception เงียบ ๆ**
(ละเมิดกฎเหล็กข้อ 2 ตรง ๆ). แก้ 2 ชั้น: พิมพ์เหตุผลออกมาเสมอ + **ยืนยันตัวตนจาก node
ของกล้องเอง** (`DeviceModelName`/`DeviceSerialNumber`/`GevCurrentIPAddress`) หลังเปิดกล้อง
ซึ่งไม่ขึ้นกับ layout ของ struct. `hik_camera.py` ต้องใช้ท่าเดียวกัน — **อย่าเชื่อ struct
ของ SDK เป็นแหล่งความจริงเดียว**.
