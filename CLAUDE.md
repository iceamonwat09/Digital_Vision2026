...
> ▶️ **เริ่มรอบหน้าตรงนี้ — 3 ขั้น ตามลำดับ (ไม่ต้องมีไฟ ไม่ต้องเข้าไลน์ 2 ขั้นแรก)**
> ```
> git pull            # branch claude/can-dent-detection-hikrobot-test-3zhqg9
> py -3.9 app.py      # footer ต้องขึ้น 2026.08.21-blurdiag
> ```
> **ขั้น A — วัดเกณฑ์ความเบลอของโมเดล** (5 นาที, ไม่ต้องเปิดกล้อง)
> ```
> py -3.9 blur_tolerance.py --selftest
> py -3.9 dump_defect_images.py --limit 30
> py -3.9 blur_tolerance.py --images sample_cans --model bestX.pt
> ```
> เอากลับมา: **"โมเดลทนเบลอได้ ≤ N พิกเซล"** ⇒ ใช้แทนค่าเดา `--blur-limit-mm 0.2`
>
> **ขั้น B — หาว่า 16.5 fps หายไปตรงไหน** (10 นาที) — ถ่ายรัว 5 วิ แล้วอ่านกล่อง
> **"วินิจฉัย"** ในแกลเลอรี ซึ่งจะบอกเองว่าเป็น *เครือข่าย* / *ดิสก์* / *กล้องช้าเอง*
> พร้อมวิธีแก้. ถ้าบอกว่าดิสก์ → ติ๊ก **"หยุดโมเดลระหว่างถ่ายรัว"** (ต้องตั้ง
> `HIK_BURST_PAUSE_INFERENCE = True` ใน `config.py` ก่อน) แล้วถ่ายซ้ำเทียบกัน
>
> **ขั้น C — ถ่ายกระป๋องนิ่งที่จุดตรวจจริง** (ปุ่ม "ถ่าย 1 เฟรม" — ไม่มีการเคลื่อนที่
> จึงตั้ง exposure ยาวได้ ภาพคมแน่นอน) เอากลับมา: **mm/px จริง** (ถ่ายไม้บรรทัดคู่กัน) ·
> **ตาเปล่ามองเห็นรอยบุบในภาพไหม** (ข้อนี้สำคัญที่สุด — จำลองได้ SNR 0.4 = แทบมองไม่เห็น) ·
> ผลตรวจของ `bestX.pt` บนภาพจากกล้องใหม่
>
> ⛔ **ยังทำไม่ได้:** วัดความแม่นที่ความเร็วไลน์ — ที่ exposure 2003 µs ภาพเบลอ ~20 px
> ไม่ว่าโมเดลไหนก็ตรวจไม่ได้ ผลจะบอกแค่ "เบลอ" ซึ่งเรารู้อยู่แล้ว
>
> ▶️ **งานที่ค้างจากรอบก่อน: ทดสอบ "ถ่ายรัว" กับกล้องจริงให้ครบ**
> ```
> py -3.9 app.py      # footer ต้องขึ้น 2026.08.21-blurdiag
> ```
...
- Tests: `pytest tests/` — **694 ผ่าน / 5 fail (pre-existing)** (artwork/label/barcode/auth —
