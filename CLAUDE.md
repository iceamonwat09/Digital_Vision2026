...
+ ms/เฟรม · สรุป + วิธีแก้) แล้วบอก **ต้นเหตุที่ตรงตัว** — เอาผลกลับมา
...
**เทสต์เพิ่ม 5 ตัวใน `tests/test_accel_backend.py` (รวม 16)** — และ**พิสูจน์ว่าจับ
...
- Tests: `pytest tests/` — **795 ผ่าน / 5 fail (pre-existing)** (artwork/label/barcode/auth —
  ...
  **`tests/test_accel_backend.py` 16 ตัว** (ตัวตรวจ export ล้าสมัยด้วย hash +
