...
## 5.2 ฉลากหลายภาษา — Gemini แทนคำด้วยภาษาอื่น (25 ก.ย. 2026)

**สคริปต์เต็มที่พร้อมวาง:** [`docs/n8n/code_in_javascript2.js`](n8n/code_in_javascript2.js)
(ต่างจากของสถานีแค่ 2 จุด: กฎข้อ 6 ใน `SYSTEM_INSTRUCTION` + ประโยคเดียวใน `PROMPT`)

**อาการที่เกิดจริง** (คู่ Friskies Lil Lickables: masterfile ↔ ปรู๊ฟ Huhtamaki) —
ทั้งสองไฟล์พิมพ์บรรทัดเกาหลีเหมือนกันทุกตัว (ยืนยันจาก text layer + ภาพ):

| | Gemini อ่านได้ |
|---|---|
| ของจริง | `제품에 별도 표시 (일월년 순) 중량: 56 g (14g × 4개입)` |
| ไฟล์หลัก | `ผลิตภัณฑ์에 별도 표시(วันเดือนปี) น้ำหนักสุทธิ: …` ← แทน 3 คำ |
| ปรู๊ฟ | `ผลิตภัณฑ์에 별도 표시 (일월년 순) 중량: …` ← แทน 1 คำ |

⇒ MISMATCH ปลอม 1 รายการ. **กฎ "NEVER translate" มีอยู่แล้วในข้อ 4** แต่อาการนี้
ไม่ใช่การแปลทั้งบรรทัด เป็นการ **แทนคำด้วยคำที่ความหมายเดียวกันจากภาษาอื่นที่พิมพ์
อยู่บนฉลากเดียวกัน** ⇒ ข้อ 6 เขียนถึงอาการนี้ตรง ๆ

**"อ่านซ้ำ 2 รอบ" ช่วยไม่ได้** (ยืนยันบนสถานีแล้ว) — `temperature: 0` ทำให้แทนคำ
แบบเดิมทุกครั้ง ⇒ ผ่านการตัดกันสองรอบไปได้

⚠️ **ตัวอย่างในข้อ 6 ห้ามใช้บรรทัดที่พลาดจริง** — ใช้ `보관방법` ซึ่งไม่มีบนฉลาก
ทดสอบ. ถ้าใส่ `제품에 별도 표시` ลงไป โมเดลจะเห็นเฉลยใน prompt แล้วการทดสอบ
บนไฟล์คู่นี้จะวัดอะไรไม่ได้

**ทดสอบบนสถานี — เปลี่ยนทีละอย่าง:**

| รอบ | เปลี่ยน | ต้องได้ |
|---|---|---|
| 1 | วางสคริปต์ใหม่ (ข้อ 6) | z2/b7 อ่านเป็น `제품에 별도 표시 (일월년 순) 중량` · MISMATCH กลุ่ม B หาย · รัน 2 ครั้ง |
| 2 | *(ถ้ารอบ 1 ยังไม่หาย)* `thinkingBudget: 1024` | ผลเดียวกัน + จดเวลาต่อโซน |
| 3 | *(แยกรอบ)* ข้อ 5 → "omit it" (ค้างมาจากหัวข้อ 5.1) | ห้ามรวมกับรอบ 1 ไม่งั้นแยกไม่ออก |

**A/B prompt เดิม ↔ prompt ใหม่ (ต่างกันตัวแปรเดียว):**
prompt เดิมอยู่ที่ [`docs/n8n/code_in_javascript2_old.js`](n8n/code_in_javascript2_old.js)
(คัดลอกตรงตัวจากสถานี · ต่างจากรุ่นใหม่แค่ข้อ 6 + วลีเดียวใน PROMPT)

1. เปิด **งานที่ค้างไว้ชุดเดิม** (ห้ามลากโซนใหม่ · ห้ามอัปโหลดใหม่) ⇒ ภาพที่ส่งให้ Gemini เหมือนเดิมทุกไบต์
2. วาง `code_in_javascript2_old.js` ในโหนด → Save → กด "ส่งตรวจสอบ" → จดผล
3. วาง `code_in_javascript2.js` → Save → กด "ส่งตรวจสอบ" อีกครั้ง → จดผล
4. จดต่อโซน: คำที่ถูกแปล (อักษรไทยปนเกาหลีในคำเดียว) · คำที่อ่านผิดแบบอื่น · จำนวนการ์ด

> 📊 **ผล A/B บนสถานี 25 ก.ย. (Friskies · งานค้างชุดเดียวกัน · ต่างกันแค่ prompt)**
> โซน 3/4/5 ได้ข้อความ **ตรงกันทุกตัวอักษร** ทั้งสองรอบ ⇒ ภาพเดียวกันจริง + Gemini นิ่ง
> · prompt ใหม่รัน 2 ครั้ง (18:3x · 18:51) ได้ข้อความ **เหมือนกันทุกตัว 10/10 โซน** ⇒ รันครั้งเดียวต่อ prompt ก็พอ
>
> | | prompt เดิม | prompt ใหม่ (ข้อ 6) |
> |---|---|---|
> | คำ/วลีเกาหลีที่ถูกแปลเป็นไทย | **4** (`ผลิตภัณฑ์에` · `ผลิตภัณฑ์ที่ระบุแยกต่างหาก` · `(วันเดือนปี)` · `น้ำหนัก`) | **1** (`ผลิตภัณฑ์에`) |
> | `리커블` อ่านผิด | ฝั่งปรู๊ฟ `리커버블` | ฝั่ง masterfile `리커버` |
> | ขยะอื่น | `420_kcal` · `42~kcal` | — |
> | การ์ด | 6 | 7 (แตกใบเพราะเว้นวรรค — ปัญหาฝั่งโค้ด ไม่ใช่ prompt) |
>
> ⇒ **ใช้ prompt ใหม่** · คำ `리커블` เป็นจุดที่ Gemini อ่านผิดเองทั้งสอง prompt
> (คนละฝั่ง) ⇒ **ไม่ใช่ผลข้างเคียงของข้อ 6** อย่างที่สงสัยตอนแรก

**กันถอยหลังทุกรอบ:** รันคู่ที่เคยผลถูกซ้ำ — AvoDerm Master1/2 ต้องได้ 2 รายการ
(`Copper Sulfate` + `D-Calcium`) · PURINA ONE ได้ 6-7 เท่าเดิม

> 🔑 prompt **ลดโอกาส** ได้แต่รับประกันไม่ได้ (กฎห้ามแปลมีอยู่แล้วก็ยังเกิด) ⇒
> ฝั่ง Python ต้องมีตัวกันแบบตายตัวคู่กัน — ใช้ text layer ของไฟล์หลักเป็นพยาน
> ตรวจบรรทัดที่ OCR อ่าน (ออกแบบ + วัดแล้ว ยังไม่ได้ทำ)
