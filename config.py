...
# (MV-CS050-10GC = 2448x2048).
# ครอปในกล้องช่วยลดแบนด์วิดท์ GigE และเพิ่ม fps จริง (ไม่ใช่แค่ครอปทีหลัง)
# เช่น ครอปเหลือ 1024x1024 = ลดข้อมูลลง ~4.8 เท่า ⇒ ปล่อยเฟรมเรตสูงได้โดยไม่ตันลิงก์
# ⚠️ ตั้งเมื่อรู้แล้วว่าฝากระป๋องอยู่ตรงไหนของเฟรม (ต้องจัดตำแหน่งกล้องนิ่งก่อน)
# มิฉะนั้นจะครอปโดนขอบกระป๋องหายโดยไม่รู้ตัว
HIK_ROI = None

# ── จำกัดเฟรมเรต (หน่วย fps) — ⚠️ อย่าตั้งเป็น None บนกล้องตัวนี้ ──────────
# วัดจากสถานีจริง (MVS client, 13 ส.ค. 2026) กับ MV-CS050-10GC:
#   2448x2048 @ 23.21 fps = **930.9 Mbps = 93% ของลิงก์ 1Gbps** → MVS รายงาน
#   Packets Lost 3066 (Errors 0 เพราะขอส่งซ้ำทัน แต่แปลว่าลิงก์ตันจริง)
#   (930.9 Mbps ตรงกับ 8 bit/พิกเซล เป๊ะ ⇒ กล้องส่ง Bayer 8-bit ไม่ใช่ RGB)
# โหมดถ่ายรูปตรวจไม่ต้องการ 23 fps เลย — viewfinder แค่ให้เล็งได้ ส่วนชัตเตอร์
# หยิบเฟรมล่าสุดที่มีอยู่แล้ว. ลดเหลือ 10 fps = 401 Mbps (40% ของลิงก์)
# ⇒ หมด packet loss + ลดภาระ debayer ของ CPU + **ไม่ไปบีบเครือข่ายโรงงาน**
# ⚠️ กล้องเสียบอยู่บนวงเดียวกับระบบงาน (กล้อง 172.32.1.253 / การ์ดแลน
#    172.32.1.9 / SQL Server 172.32.0.50) — ปล่อยเต็ม 931 Mbps จะกวนคนอื่นด้วย
# ตั้ง None = ไม่จำกัด (ใช้ค่ากล้อง) — ทำได้ต่อเมื่อแยกการ์ดแลนให้กล้องโดยเฉพาะ
HIK_FRAME_RATE = 10.0
...
# ── ถ่ายรัว (burst) — เครื่องมือทดสอบว่า "จับทันและนิ่งพอไหม" ──────────────
# กดครั้งเดียว เก็บเฟรมติดกันหลายใบตามอัตราที่กล้องส่งมาจริง แล้วค่อยตรวจทีหลัง
# (เก็บให้ครบก่อน แล้วค่อย infer — ถ้า infer คั่นกลางจะพลาดเฟรมช่วงที่ของวิ่งผ่าน)
# ⚠️ กินหน่วยความจำชั่วคราว = จำนวนเฟรม x ขนาดเฟรม
#    (10 x 2448x2048x3 ≈ 150 MB บน MV-CS050-10GC)
BURST_COUNT = 10
BURST_MAX_COUNT = 30          # เพดานกันตั้งเยอะจน RAM หมด
BURST_TIMEOUT_S = 6.0         # เก็บไม่ครบภายในเวลานี้ = คืนเท่าที่ได้ พร้อมบอกจำนวน
BURST_THUMB_W = 420           # ความกว้างรูปย่อที่ส่งกลับไปแสดงเป็นแถบ
