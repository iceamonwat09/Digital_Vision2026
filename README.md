...
**ด่านคุณภาพของ PDF text layer** (ตัดสินว่าโซนไหน "เชื่อข้อความใน PDF ได้" — ดูหัวข้อ
[text layer เชื่อได้แค่ไหน](#-text-layer-เชื่อได้แค่ไหน--ด่านคุณภาพ-2-ชั้น)):

| ตัวแปร | ค่า default | หน้าที่ |
|---|---|---|
| `ARTWORK_PDFTEXT_GARBLED_CHECK` | `1` | เปิด/ปิดด่านคุณภาพทั้งหมด (`0` = เชื่อ text layer เสมอ = พฤติกรรมก่อนมีด่าน) |
| `ARTWORK_PDFTEXT_BAD_GLYPH_CHECK` | `1` | **ด่านอักขระต้องห้าม** — เจออักขระควบคุม / Private-Use / `U+FFFD` แม้แต่ตัวเดียวถือว่า text layer เสีย (`0` = เหลือแต่ด่านสัดส่วนคำผิดรูป) |
| `ARTWORK_PDFTEXT_BAD_GLYPH_MIN_COUNT` | `1` | จำนวนอักขระต้องห้ามขั้นต่ำที่ทำให้ปฏิเสธ |
| `ARTWORK_PDFTEXT_GARBLED_MIN_TOKENS` | `8` | ด่านสัดส่วน: ต้องมีคำยาว ≥ ค่านี้จึงจะยอมตัดสิน (กันฟ้องรหัสงานพิมพ์) |
| `ARTWORK_PDFTEXT_GARBLED_RATIO` | `0.30` | ด่านสัดส่วน: สัดส่วนคำผิดรูปที่ถือว่าเสีย |
| `ARTWORK_OCR_GROUP_ENGINE_CONSISTENCY` | `0` | **opt-in** — กลุ่มไหนมีทั้งโซนที่อ่านจาก text layer และโซนที่อ่านด้วย OCR ให้อ่านโซน text layer ซ้ำด้วย OCR เพื่อให้เทียบกันด้วย engine เดียวกัน |
...
| `ARTWORK_CROP_MIN_SIDE` | `1200` | ด้านยาวขั้นต่ำ (px) ของภาพ crop ในการ์ด defect — โซนเล็กจะถูกเรนเดอร์ใหม่ที่ DPI สูงขึ้น เพื่อให้ Tesseract อ่านออก |
...
3. **ส่งตรวจ** — ระบบอ่านข้อความทีละโซน (PDF text layer ถ้ามี **และผ่านด่านคุณภาพ**
   ไม่งั้นยิง N8N→Gemini) แล้วรันทุกชั้นตรวจ
   - ติ๊ก **"🔍 อ่านทุกโซนด้วย OCR (ข้ามข้อความใน PDF)"** ได้เมื่อรู้ว่าไฟล์นั้น
     ฟอนต์พังจนข้อความใน PDF ใช้ไม่ได้ทั้งใบ (ปกติไม่ต้องติ๊ก — ระบบจับรายโซนให้เอง)
...
### 🔤 text layer เชื่อได้แค่ไหน — ด่านคุณภาพ 2 ชั้น

PDF ที่ยังไม่ outline จะมี "ข้อความจริง" ฝังอยู่ ระบบใช้ค่านั้นก่อนเสมอเพราะ
**แม่นระดับตัวอักษร 100% ฟรี และไม่ต้องต่อเน็ต** (ต่างจาก OCR ที่วัดได้ recall 98.4% /
precision 95.8%) — แต่ **"มีข้อความ" ไม่เท่ากับ "ข้อความใช้ได้"**

ฟอนต์ที่ถูก subset ตอน export บางตัวมีตาราง ToUnicode CMap ที่แมป glyph กลับเป็น
Unicode ไม่ได้ ⇒ `get_text()` คืนขยะออกมาโดยไม่มี error. เคสจริงที่เจอ — **ไฟล์ 2 ฉบับ
ของงานเดียวกัน เนื้อหาเดียวกันเป๊ะ ฟอนต์ชื่อเดียวกัน (`DINPro-Bold`) แต่ subset คนละชุด**:

| ไฟล์ | สิ่งที่ text layer คืน |
|---|---|
| ฉบับ Original | `DOPLŇKOVÉ KRMIVO PRO DOSPĚLÉ KOČKY` ✅ |
| ฉบับ A4 (proof) | `(340ĊKOVÉ KRMIVO PR3␄(374Ý0␌␄KOÏ/=` ❌ |

ระบบจึงตรวจคุณภาพก่อนใช้ **ทีละโซน** ด้วย 2 ด่านที่เป็นอิสระจากกัน:

| ด่าน | จับอะไร | เกณฑ์ |
|---|---|---|
| ① **อักขระต้องห้าม** | อักขระควบคุม (C0/C1), Private-Use-Area, surrogate, unassigned, `U+FFFD` | **เจอตัวเดียวก็พอ** — อักขระพวกนี้ไม่มีเหตุผลจะอยู่ในข้อความที่พิมพ์บนฉลาก |
| ② **สัดส่วนคำผิดรูป** | ตัวเลขแทรกกลางคำยาว (`PR3374Y0KOI`) | ต้องมีคำยาว ≥ 8 คำ และผิดรูป ≥ 30% จึงจะตัดสิน (กันฟ้องรหัสงานพิมพ์) |

**โซนที่ไม่ผ่านด่าน = ไม่ใช้ข้อความจาก PDF แต่ไปอ่านจากภาพด้วย OCR แทน** และถ้าไม่มี
OCR backend ให้ใช้ จะกลายเป็น `UNREADABLE` (ขอให้คนดู) แทนที่จะเงียบว่าถูกต้อง

> ⚠️ **ห้ามนับ Unicode category `Cf` เป็นอักขระต้องห้ามเด็ดขาด** — ZWJ/ZWNJ/RLM/LRM
> เป็นของปกติในข้อความอาหรับ/ฮีบรู ใส่เข้าไปจะฟ้องผิดทุกฉลากที่มีภาษาเหล่านั้น

**วัดจริงบนไฟล์คู่ข้างบน** (แบ่งหน้าเป็นตาราง 6×6 แล้วตัดสินทีละโซน):

| | โซนที่มีข้อความ | ถูกปฏิเสธ | ในนั้นด่าน ② เดิมจับได้ |
|---|---|---|---|
| ฉบับ Original (ฟอนต์ดี) | 18 | **0** — ไม่มีฟ้องผิดเลย | — |
| ฉบับ A4 (ฟอนต์พัง) | 54 | **16** | **1** (อีก 15 เคยหลุดไปหมด) |

เทียบผลตรวจโซนเดียวกันก่อน/หลังมีด่าน ①: เดิมได้ **40 defect** รวม `SPELL_FAIL "KEPEVIXGI"`
(คำที่**ไม่มีอยู่บนฉลาก** — เป็นขยะจากฟอนต์) · ตอนนี้เหลือ **1 รายการ: `UNREADABLE`**
⇒ ตรงหลักการของโปรเจกต์: *ผลที่ผิดแบบมั่นใจ แย่กว่าไม่แสดงผล*

**engine ปนกันในกลุ่มเดียวกัน:** ถ้ากลุ่มที่เอามาเทียบกันมีทั้งโซนที่อ่านจาก text layer
และโซนที่อ่านด้วย OCR ความต่างที่เห็นอาจมาจาก *วิธีอ่าน* ไม่ใช่ *งานพิมพ์* — แถบ
"ตรวจอะไรไปบ้าง" ใต้ผลสรุปจะบอกให้เสมอ (ข้อมูลประกอบ ไม่กระทบ verdict) และตั้ง
`ARTWORK_OCR_GROUP_ENGINE_CONSISTENCY=1` เพื่อให้ระบบอ่านซ้ำให้ engine ตรงกันทั้งกลุ่มได้
...
| ② | **Tesseract** (local) | ไฟล์ outline (artwork ทั่วไป) / ภาพถ่าย — พิกัด "วัดจากพิกเซลจริง" |
| ③ | `bbox` ที่ OCR backend (Gemini) คืนมา | ชั้น ①② หาไม่เจอ — **เป็นพิกัดแบบประมาณ ต้องพิสูจน์ด้วยการอ่านซ้ำก่อนถึงวาด** |
...
#### 📐 ลากโซนอย่างไรให้กรอบแดงขึ้น

กรอบแดงต้องการให้ตัวอักษรในภาพ crop สูงราว **9-20 พิกเซล** จึงจะชี้ตำแหน่งคำได้
(วัดจากไฟล์จริง — ไม่ใช่ยิ่งใหญ่ยิ่งดี: ตัวสูง 32px กลับอ่านได้น้อยกว่า)

- ✅ **ลากกระชับรอบบล็อกข้อความ** (เช่นเฉพาะตารางโภชนาการ) — ระบบชี้คำได้ครบ
- ❌ **ลากเป็นแถบกว้างทั้งแผ่น** — ภาพถูกย่อจนตัวหนังสือเหลือ ~8px ระบบหาคำไม่เจอ
  และ**เร่งความละเอียดก็ช่วยไม่ได้** เพราะแถบกว้างมีกราฟิก/ภาพถ่าย/หลายภาษาปนกัน
- 💡 **ดับเบิลคลิกที่โซน** ให้ระบบขยับกรอบให้พอดีเนื้อหาเอง

ระบบ**เตือนให้เอง 2 จุด** (ไม่กระทบผลตรวจ PASS/FAIL เลย):
1. **ตอนจัดโซน** — เลือกโซนแล้วมีบรรทัดเตือนในแผงคุณสมบัติทันทีถ้าโซนนั้นจะชี้คำไม่ได้
   (คำนวณจากรูปทรงโซนล้วน ๆ ไม่ต้องส่งตรวจก่อน) → แก้ได้เลยก่อนเสียเวลา
2. **ในการ์ด "รายการที่พบ"** — ถ้า defect นั้นไม่มีกรอบแดงเพราะโซนกว้าง/เล็กเกินไป
   จะบอกเหตุผล + วิธีแก้ไว้ใต้รูป

จุดที่ยังอ่อน: โลโก้/badge ที่เป็นตัวอักษรสไตล์ไลซ์บนกราฟิก (มักไม่ใช่สิ่งที่ถูกฟ้องอยู่แล้ว)

#### 🔍 กรอบไม่ขึ้น / ขึ้นผิดที่ — ตรวจด้วย `diagnose_highlight.py`

รันบนเครื่อง server (ไม่ต้องปิด `app.py`) โดยใส่ id ของการตรวจที่มีปัญหา:

```bash
py -3.9 diagnose_highlight.py                                 # ใช้การตรวจล่าสุด
py -3.9 diagnose_highlight.py 20260731-152807-632ed0          # ระบุรายการเอง
py -3.9 diagnose_highlight.py 20260731-152807-632ed0 --save   # เขียนรูปที่วาดกรอบออกมาดู
```

> อ่านอย่างเดียว ไม่แก้ไขรายงานที่มีอยู่

จะบอกให้ครบว่า: ค่า config ที่ใช้อยู่ · เจอ `tesseract.exe` ที่ไหนและมีภาษาอะไรบ้าง ·
แต่ละโซนใช้ชั้นไหนหาคำ · แต่ละ defect ได้กี่กรอบ และ **อ่านซ้ำในกรอบว่าเป็นคำอะไรจริง ๆ**
(ใช้พิสูจน์ว่ากรอบตรงคำหรือไม่ โดยไม่ต้องเพ่งรูปเอง)

เช็คลิสต์เร็ว ๆ เมื่อกรอบไม่ขึ้น:

| อาการ | สาเหตุที่พบบ่อย | วิธีแก้ |
|---|---|---|
| ไม่มีกรอบทั้งรายงาน | ไม่ได้ติดตั้ง `pytesseract` / tesseract binary | ดูขั้นตอนติดตั้งด้านบน แล้ว **ส่งตรวจใหม่** |
| ไม่มีกรอบเฉพาะบางโซน | โซนกว้าง/เล็กเกินไป (มีคำเตือนใต้รูปอยู่แล้ว) | ลากโซนให้กระชับ แล้วส่งตรวจใหม่ |
| ไม่มีกรอบเฉพาะคำอาหรับ/จีน | ไม่ได้ติดตั้ง traineddata ของภาษานั้น | ติดตั้งภาษาเพิ่ม + ตั้ง `ARTWORK_HIGHLIGHT_TESS_LANG` |
| รายงานเก่าไม่มีกรอบ | รายงานที่ตรวจก่อนอัปเดตไม่มีข้อมูลที่เก็บเพิ่ม | **ส่งตรวจใหม่** (ของเดิมไม่ย้อนหลัง) |
...
- `POST /api/artwork/<id>/inspect` — ตรวจทุกชั้น
  (body: `zones`, `brand`, `auto_rotate`, **`force_ocr`** = ข้ามข้อความใน PDF อ่านด้วย OCR ทั้งใบ)
...
├── diagnose_highlight.py        # เครื่องมือตรวจ "ทำไมกรอบแดงไม่ขึ้น/ขึ้นผิดที่" ของโหมด Artwork
...
### 🔤 Artwork: อุดช่องโหว่ "ข้อความใน PDF เชื่อไม่ได้" (1 ก.ย. 2026)

`CONFIG_VERSION = 2026.09.01-glyphguard`

ผู้ใช้ส่งไฟล์จริง 2 ฉบับของงานเดียวกันมา — **เนื้อหาเดียวกันเป๊ะ ฟอนต์ชื่อเดียวกัน
(`DINPro-Bold`) แต่ subset คนละชุด** — แล้ว text layer อ่านได้ไม่เหมือนกัน
(`DOPLŇKOVÉ KRMIVO…` vs `(340ĊKOVÉ KRMIVO PR3␄(374Ý0␌…`) ⇒ ToUnicode CMap ของ
subset ในไฟล์หนึ่งแมป glyph กลับเป็น Unicode ไม่ได้

- 🚨 **ด่านเดิมจับไม่ได้ — วัดแล้ว ไม่ใช่เดา**: คำผิดรูป **2/14 คำยาว = 14.3%** ต่ำกว่า
  เกณฑ์ 30% และคำที่พังบางคำ**ไม่มีตัวเลขเลย** ⇒ กฎ "ตัวเลขแทรกกลางคำ" ไม่มีทางจับได้
- ✅ **ด่านใหม่: อักขระต้องห้าม** (control / Private-Use / surrogate / unassigned /
  `U+FFFD`) — **เจอตัวเดียวก็ปฏิเสธ** ไม่ต้องรอสัดส่วน · `ARTWORK_PDFTEXT_BAD_GLYPH_CHECK=0`
  เพื่อ rollback
- ⚠️ **ห้ามนับ category `Cf`** (ZWJ/RLM ของอาหรับ/ฮีบรู) — มีเทสต์ + mutation test ล็อกไว้
- 🔍 **ช่องติ๊ก "อ่านทุกโซนด้วย OCR (ข้ามข้อความใน PDF)"** สำหรับไฟล์ที่ฟอนต์พังทั้งใบ
  — สั่งแล้วแต่ไม่มี OCR backend = **ไม่บังคับ** (คงข้อความที่อ่านได้ไว้ ดีกว่าได้ UNREADABLE)
- 📋 **แถบ "ตรวจอะไรไปบ้าง" บอกเมื่อกลุ่มหนึ่งเทียบข้าม engine** (text layer ปนกับ OCR)
  เพราะความต่างที่เห็นอาจมาจากวิธีอ่าน ไม่ใช่งานพิมพ์ · เปิด
  `ARTWORK_OCR_GROUP_ENGINE_CONSISTENCY=1` ให้ระบบอ่านซ้ำให้ engine ตรงกันทั้งกลุ่มได้
- 🧪 **`verify_ocr.py` เลิกใช้ text layer ที่เสียเป็นเฉลย** — เดิมหักคะแนน engine ที่อ่าน*ถูก*
- 📊 **วัดบนไฟล์จริง**: ฉบับฟอนต์ดี **ปฏิเสธ 0/18 โซน** (ไม่มีฟ้องผิด) · ฉบับฟอนต์พัง
  **ปฏิเสธ 16 โซน** ซึ่งด่านเดิมจับได้เพียง 1 · A/B ระดับ HTTP โซนเดียวกัน: เดิมได้
  **40 defect** รวม `SPELL_FAIL "KEPEVIXGI"` (คำที่ไม่มีอยู่บนฉลาก) → ตอนนี้เหลือ
  `UNREADABLE` 1 รายการ
- ✅ เทสต์ใหม่ 43 ตัว · pytest **1081 ผ่าน / 5 fail เดิม** · ย้อนโค้ด 6 จุด เทสต์แดงทุกจุด

### 🖍️ Artwork: กรอบแดง "ขึ้นบ้างไม่ขึ้นบ้าง" — ปิดช่องความไม่แน่นอน (7 ส.ค. 2026)

จากรายงานว่า "ทำได้แล้ว แต่ไม่ใช่ทุกครั้ง" → ไล่วัดทุกเคสจนเจอสาเหตุจริง แล้วแก้เฉพาะ
จุดที่พิสูจน์ได้ **ทุกอย่างยังเป็น display-only ไม่แตะผลตรวจ PASS/FAIL**

- 🔢 **ตัวเลขอาหรับ-อินดิก (`٤٧٥`) จับคู่กับ `475` ได้แล้ว** — เดิม Tesseract โหมด `ara`
  คืนเลขคนละชุดกับที่ defect ฟ้อง จึงไม่มีกรอบแบบเงียบ ๆ (ไม่มี error ให้เห็น)
- 🎯 **เลิกเดาแถวเมื่อกำกวม** — การเลือกบรรทัดตอนอ่านซ้ำเปลี่ยนจาก substring เป็น
  **ทั้งคำ** และถ้าคำนั้นอยู่หลายบรรทัด = ชี้แถวไม่ได้ → **ไม่วาด** (เดิมเคสจริง `0`
  จาก `0 g` ไปตรงกับ `10%` คนละแถว)
- ✂️ **คำที่ขอบโซนตัดผ่านไม่ถูกวาดอีกต่อไป** — PDF คืนกรอบเต็มคำบนหน้ากระดาษ แม้อยู่ใน
  โซนแค่เสี้ยวเดียว ทำให้ได้แถบบางติดขอบที่ดูเหมือนวาดผิดที่ (ทิ้งเมื่อหลุดโซน >25%)
- 🔒 **cache OCR ปลอดภัยต่อ multi-thread** — Flask `threaded=True` ทำให้ 2 request
  ชนกันได้จริง; เปลี่ยนเป็น `OrderedDict` + lock (LRU 12 รายการ)
- ⚠️ **เตือนผู้ใช้ว่าโซนนี้จะชี้คำไม่ได้ — 2 จุด**: (1) **ตอนจัดโซน** คำนวณจากรูปทรงโซน
  ล้วน ๆ (ฟรี ไม่ต้องเรนเดอร์/OCR) ขึ้นเตือนทันทีที่เลือกโซน (2) **ในการ์ดผลตรวจ**
  บอกเหตุผล + วิธีแก้ใต้รูปที่ไม่มีกรอบ
- 📏 **ข้อจำกัดที่วัดได้ (แก้ด้วยโค้ดไม่ได้)**: กรอบแดงต้องการตัวอักษรสูง ~9-20px ในภาพ
  crop. โซนที่ลากเป็นแถบกว้างทั้งแผ่นเหลือ ~8px → หาคำไม่เจอ และ**เร่งความละเอียด
  ก็ตันที่ 6/14** ส่วนโซนกระชับรอบตารางเดียวกันได้ **14/14** → จึงเลือกทางเตือนผู้ใช้
- 🧪 **วัดแล้วปฏิเสธ 3 แนวทางของตัวเอง** (ไม่ใส่เข้าระบบ): ปิด dictionary ของ Tesseract
  = ไม่มีผล · Sauvola thresholding = แย่ลง · โหลดหลายภาษาพร้อมกัน = ความแม่นเท่าเดิม
  แต่ช้าลง ~3.5 เท่า. และพบว่า **ย่อภาพ raster ลงคือหายนะ** (1/14)
- ✅ regression: e2e production path กับ artwork จริง 6 โซน ยัง **25/25** เท่าเดิม
...
- **หาตำแหน่งแบบ 4 ชั้น** (ชั้นแรกที่เจอชนะ):
  ① **พิกัดคำจาก PDF text layer** (เป๊ะระดับ vector, ทุกภาษา, ไม่ต้อง OCR) →
  ② **Tesseract** (ไฟล์ outline / ภาพถ่าย — วัดจากพิกเซลจริง) →
  ③ `bbox` จาก OCR backend (**ประมาณ** — ต้องพิสูจน์ด้วยการอ่านซ้ำก่อนวาด) → ④ ไม่วาด
...
ชุดเทสต์ครอบคลุมทั้งโหมดฉลากกระดาษ, Artwork และกล้องอุตสาหกรรม
(**1081 ผ่าน / 5 fail เดิม** — ดูหมายเหตุท้ายตาราง):

| ไฟล์ | ครอบคลุม |
|---|---|
| `test_artwork_checks.py` | 4 ชั้นตรวจ Artwork, ความทนต่อ OCR noise, snap-to-content, ตารางคำแปล + คำแนะนำ + cache |
| `test_artwork_ocr_quality.py` | ด่านคุณภาพของ text layer: เพิ่ม DPI ให้โซนเล็ก, จับฟอนต์ที่แมปอักขระผิดทั้ง 2 ด่าน (อักขระต้องห้าม + สัดส่วนคำผิดรูป), **กันฟ้องผิดกับอาหรับ/ฮีบรู/ไทย/จีน**, ปิด flag แล้วกลับพฤติกรรมเดิม |
| `test_artwork_engine_source.py` | "ข้อความมาจากไหน": `force_ocr` (ข้าม text layer), บังคับไม่ได้ต้องไม่ทำให้แย่ลง, รายงานกลุ่มที่เทียบข้าม engine, การอ่านซ้ำให้ engine ตรงกันทั้งกลุ่ม |
| `test_artwork_coverage.py` | รายงานว่าชั้นไหน "ได้ทำงานจริง" กับงานใบนี้ (PASS ไม่ได้แปลว่าตรวจครบ) |
| `test_artwork_ocr_cache.py` | cache ของแท็บคำแปลต้องหลุดเมื่อค่าตั้งที่เปลี่ยนผลการอ่านเปลี่ยน |
| `test_artwork_pixdiff.py` | เทียบภาพเก่า/ใหม่ระดับพิกเซล + ด่านปฏิเสธเมื่อเทียบไม่ได้ |
| `test_artwork_report_css.py` | คลาส CSS ที่ `renderReport()` พ่นออกมา ต้องมีครบทั้ง 2 template |
| `test_artwork_highlight.py` | กรอบแดงชี้คำผิด: หาตำแหน่งคำ, แปลงพิกัด bbox 3 convention, พิกัดคำจาก PDF text layer, การหมุนกรอบ, วาดทุกจุด + cap, auto-detect tesseract, กรองภาษา, กันกรอบผิดคำสั้น/CJK, ตัวเลขอาหรับ-อินดิก, ปฏิเสธแถวกำกวม, ทิ้งกรอบที่ขอบโซนตัด, cache multi-thread, เตือนโซนที่ชี้คำไม่ได้ |
...
> (ตารางนี้แสดงเฉพาะไฟล์หลัก — ดูรายการเต็มใน `tests/` และใน CLAUDE.md)
