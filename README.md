...
| `ARTWORK_PDFTEXT_FONT_EVIDENCE` | `chars` | **หลักฐานระดับฟอนต์** — ฟอนต์ที่ถูกพิสูจน์ว่าแมปอักขระผิดที่ใดที่หนึ่งในไฟล์ จะไม่ถูกเชื่อในที่อื่นด้วย · `off` = ปิด · `nonascii`/`font` = เข้มขึ้น |
| `ARTWORK_PDFTEXT_FONT_EVIDENCE_MAX_PAGES` | `12` | จำนวนหน้าสูงสุดที่ไล่เก็บหลักฐาน (~60 ms/หน้า) |
| `ARTWORK_PDFTEXT_FONT_STRUCTURE_CHECK` | `1` | **หลักฐานเชิงโครงสร้าง** — สงสัยฟอนต์ที่ฝังมาแบบ CID/`Identity-H` และไม่มี `/ToUnicode` (ตามสเปก PDF แล้วถอดกลับเป็นตัวอักษรไม่ได้) โดยไม่ต้องรอเห็นอาการ · `0` = ใช้แต่หลักฐานจากอาการ |
...
| ③ **หลักฐานระดับฟอนต์** | ข้อความของฟอนต์ที่เชื่อไม่ได้ — รู้ได้ 2 ทาง: **จากอาการ** (ด่าน ① เคยจับฟอนต์นั้นได้ที่อื่นในไฟล์) หรือ **จากโครงสร้าง** (ฟอนต์เป็น CID/`Identity-H` ที่ไม่มี `/ToUnicode`) | ทำงานเฉพาะเมื่อมีฟอนต์ที่เข้าเงื่อนไขอย่างน้อยหนึ่งทาง |
...
**ทำไมต้องมีด่าน ③ ทั้งที่มี ① แล้ว** — ฟอนต์ที่พังตัวเดียวกันคายขยะได้ 2 แบบ
และแบบที่สองตรวจจากตัวข้อความไม่ได้เลย:

```
บรรทัดเดียวกัน แบ่งเป็น 2 span:
  [0] 'DOPLNKOVÉ KRMIVO PRE DOSPELÉ MA'   ← อักขระต้องห้าม 0 ตัว
  [1] 'Ï/='                                ← อักขระต้องห้าม 0 ตัว (ของจริงคือ "ČKY")
```

ความเสียหายเกาะอยู่กับ **ฟอนต์** (วัดจากไฟล์จริง: `DINPro-Bold` เสีย 66/396 span
ส่วนอีก 12 ฟอนต์ในไฟล์เดียวกัน **0%**) ⇒ ระบบจึงอ่าน span ทั้งเอกสารครั้งเดียว
หาว่าฟอนต์ไหนมีหลักฐานแข็ง แล้ว**ไม่เชื่อข้อความของฟอนต์นั้นในที่อื่นด้วย**

> 🔑 **ชั้นนี้ทำงานเฉพาะเมื่อมีหลักฐานแล้วเท่านั้น** ⇒ ไฟล์ที่ฟอนต์ปกติทั้งหมด
> **ไม่ถูกแตะแม้แต่โซนเดียว** (ยืนยันกับไฟล์ที่ฟอนต์ดี: ปฏิเสธ 0/18 โซน)

**ต้นเหตุระดับสเปก PDF** — ไฟล์คู่ที่ใช้ทดสอบต่างกันตรงนี้:

| | Original (อ่านได้) | A4 (พัง) |
|---|---|---|
| ฟอนต์หัวเรื่อง | Type1 + `WinAnsiEncoding` | Type0 + `Identity-H` |
| `/ToUnicode` | ไม่มี | ไม่มี |

`WinAnsiEncoding` เป็น encoding มาตรฐาน ⇒ ถอดกลับเป็นตัวอักษรได้เอง ส่วน
`Identity-H` แปลว่า *"เลขที่เขียนคือหมายเลข glyph ในซับเซ็ตนี้"* ⇒ ถ้าไม่มี
`/ToUnicode` ก็ **ไม่มีข้อมูลใดในไฟล์บอกได้ว่า glyph นั้นคือตัวอักษรอะไร**
(PDF/UA และ PDF/A-2u บังคับให้มีด้วยเหตุผลนี้) ⇒ ระบบจึงตรวจเงื่อนไขนี้โดยตรง
และรู้ได้ **ก่อนเห็นข้อความเสียสักตัว**

> ⚠️ **"ไม่มี ToUnicode" อย่างเดียวใช้เป็นเกณฑ์ไม่ได้** — ไฟล์จริงทั้งสองฉบับ
> **0/14 ฟอนต์มี ToUnicode** แต่ส่วนใหญ่อ่านถูก. ตัวชี้ขาดคือ `Identity-H`

**รายงานบอกชื่อฟอนต์ที่ผิดให้ด้วย** (`report.json["font_trust"]` และแถบ
"ตรวจอะไรไปบ้าง") — ต้นเหตุอยู่ที่ขั้นตอน export ของคนทำ artwork ไม่ใช่ที่ระบบตรวจ
⇒ ส่งชื่อฟอนต์กลับไปให้ผู้ออกแบบ export ใหม่ได้ตรงจุด

#### 📤 ขอไฟล์แบบไหนจากผู้ออกแบบ จึงจะตรวจได้แม่นที่สุด

ปัญหาทั้งหมดข้างบนเกิดที่ **ขั้นตอน export** ไม่ใช่ที่ระบบตรวจ — แก้ที่ต้นทาง
ถูกกว่าไล่ตรวจปลายทางเสมอ. สิ่งที่ควรขอ เรียงตามผลที่ได้:

| ขอแบบไหน | ผลกับการตรวจ |
|---|---|
| **PDF ที่ยังมี text layer + ฝังฟอนต์ครบ** (เช่น export เป็น **PDF/X-4**) | ✅ ดีที่สุด — อ่านข้อความได้แม่นระดับตัวอักษร ฟรี ไม่ต้องพึ่ง OCR |
| PDF ที่ **outline ตัวอักษรทั้งหมด** | ⚠️ ใช้ได้ — ไม่มี text layer ให้พัง แต่ทุกโซนต้องอ่านด้วย OCR (ช้ากว่า แม่นน้อยกว่า) |
| PDF ที่ฝังฟอนต์แบบ **CID/`Identity-H` โดยไม่มี `/ToUnicode`** | ❌ แย่ที่สุด — "ดูเหมือนมีข้อความ" แต่ข้อความที่อ่านได้เชื่อไม่ได้ (เคสในหัวข้อนี้) |

> 💡 **ประโยคที่ส่งกลับไปให้ผู้ออกแบบได้เลย:** *"ไฟล์นี้ฝังฟอนต์ `<ชื่อฟอนต์>`
> แบบที่ถอดกลับเป็นตัวอักษรไม่ได้ (CID/Identity-H ไม่มีตาราง ToUnicode) รบกวน
> export ใหม่เป็น PDF/X-4 โดยฝังฟอนต์ให้ครบ หรือ outline ตัวอักษรทั้งหมด"*

ในวงการงานพิมพ์ ปัญหานี้มีชื่อเรียกเป็นทางการว่า *"text cannot be mapped to
Unicode"* และเป็นข้อบังคับของมาตรฐาน **PDF/UA** และ **PDF/A-2u** — เครื่องมือ
preflight อย่าง **Enfocus PitStop**, **callas pdfToolbox**, **veraPDF** หรือ
accessibility checker ของ **Adobe Acrobat** ตรวจข้อนี้ได้ (บางตัวแก้ให้ได้ด้วย)
⇒ ถ้าโรงพิมพ์/เอเจนซีมีเครื่องมือพวกนี้ ให้เพิ่มเป็นด่านก่อนส่งไฟล์มาตรวจ
...
### 🧾 Artwork: ตรวจที่ต้นเหตุระดับสเปก PDF + เทียบ 4 แนวทางด้วยตัวเลข (1 ก.ย. 2026 รอบ 3)

`CONFIG_VERSION = 2026.09.01-cidguard`

วัด 4 แนวทางบนไฟล์จริง **833 บรรทัด (เสียจริง 70)** โดยใช้ไฟล์ Original เป็นเฉลย:

| วิธี | จับได้ | ฟ้องผิด | recall | precision | F1 |
|---|---|---|---|---|---|
| ด่านอักขระต้องห้ามอย่างเดียว | 58 | **0** | 82.9% | **100%** | 90.6% |
| + หลักฐานฟอนต์ **จากอาการ** | 70 | 10 | 100% | 87.5% | 93.3% |
| **pdfminer `(cid:N)`** | 70 | **73** | 100% | 49.0% | 65.7% |
| **+ หลักฐานฟอนต์จากโครงสร้าง (ที่ใช้)** | **70** | **10** | **100%** | **87.5%** | **93.3%** |

- 🧾 **หลักฐานทางที่ 2 จากโครงสร้างไฟล์**: ฟอนต์ CID/`Identity-H` ที่ไม่มี
  `/ToUnicode` = ตามสเปกแล้วถอดกลับไม่ได้ ⇒ สงสัยได้ทันทีโดยไม่ต้องรอเห็นขยะ
  (`ARTWORK_PDFTEXT_FONT_STRUCTURE_CHECK=0` เพื่อปิด)
- 🎯 **เพิ่ม recall เชิงหลักการฟรี — ฟ้องผิดเท่าเดิมเป๊ะ (10)** · ราคา ~6 ms/ไฟล์
- 🕳️ **ปิดจุดตายของทางที่ 1**: ถ้าฟอนต์พังแล้วบังเอิญคายแต่ตัวอักษรที่ดูปกติ
  หลักฐาน "จากอาการ" จะตาบอดสนิท (ทดสอบแล้ว: suspect = 0) ส่วนทางนี้ยังจับได้
- ⛔ **ปฏิเสธ pdfminer เป็นตัวตัดสิน** — ฟ้องผิด 73 บรรทัด โดย **71 เป็นฟอนต์ที่ดี**
  (`(cid:N)` แปลว่า "pdfminer แมปไม่ได้" ไม่ใช่ "ไฟล์แมปไม่ได้") + ช้ากว่า **28 เท่า**
- ✅ เทสต์ 35 ตัว · pytest **1118 ผ่าน / 5 fail เดิม** · ย้อนโค้ด 6 จุด แดงทุกจุด

### 🧬 Artwork: หลักฐานระดับฟอนต์ — "พังครั้งหนึ่ง = สงสัยทั้งไฟล์" (1 ก.ย. 2026 รอบ 2)

`CONFIG_VERSION = 2026.09.01-fonttrust`

ด่านอักขระต้องห้าม (รอบแรก) ยังปิดไม่หมด — ฟอนต์ที่พังตัวเดียวกันคายขยะได้ 2 แบบ
และแบบที่สอง (`MAČKY` → `MAÏ/=`) **ไม่มีอักขระต้องห้ามเลย** จึงตรวจจากตัวข้อความไม่ได้

- 🧬 **`artwork_check/fonttrust.py`** (ใหม่) — อ่าน span ทั้งเอกสารครั้งเดียว
  (~60-180 ms) หาว่าฟอนต์ไหนมีหลักฐานแข็ง แล้วไม่เชื่อข้อความของฟอนต์นั้นในที่อื่น
- 📊 **ความเสียหายเกาะกับฟอนต์จริง (วัดแล้ว)**: `DINPro-Bold` เสีย 66/396 span ·
  อีก 12 ฟอนต์ในไฟล์เดียวกัน **0%** · ไฟล์ที่ฟอนต์ดี = ไม่มีฟอนต์ต้องสงสัยเลย
- 🎚️ **4 โหมด** (`ARTWORK_PDFTEXT_FONT_EVIDENCE`): `off` / **`chars`** (ค่าเริ่มต้น —
  จับเพิ่ม 46 span ฟ้องผิด ~5 ⇒ precision ~90%) / `nonascii` / `font`
- 🔑 **ทำงานเฉพาะเมื่อมีหลักฐานแล้ว** ⇒ ไฟล์ปกติไม่ถูกแตะแม้แต่โซนเดียว
- 🏷️ **รายงานบอกชื่อฟอนต์ที่ผิด** เพื่อให้ขอไฟล์ที่ export ใหม่ได้ตรงจุด
- ⚠️ **กับดักที่เจอตอนวัด**: ถ้าคำนวณชุด "อักขระที่เปื้อน" จาก span สะอาดของฟอนต์
  ตัวเอง ตัวขยะจะลบตัวเองออกจากรายการ ⇒ ชั้นนี้จับได้ 0 span — ฐานเทียบต้องเป็น
  **ฟอนต์อื่นที่ไม่มีร่องรอย** (มีเทสต์ล็อกไว้)
- ✅ เทสต์ใหม่ 23 ตัว · pytest **1105 ผ่าน / 5 fail เดิม** · ย้อนโค้ด 8 จุด แดงทุกจุด

### 🔤 Artwork: อุดช่องโหว่ "ข้อความใน PDF เชื่อไม่ได้" (1 ก.ย. 2026 รอบ 1)
...
(**1118 ผ่าน / 5 fail เดิม** — ดูหมายเหตุท้ายตาราง):
...
| `test_artwork_fonttrust.py` | หลักฐานระดับฟอนต์ 2 ทาง (จากอาการ + จากโครงสร้าง `Identity-H`), จับขยะที่ไม่มีอักขระต้องห้าม, ไฟล์ปกติต้องไม่ถูกแตะ, ฐานเทียบต้องมาจากฟอนต์อื่น, 4 โหมดความเข้ม, เกณฑ์โครงสร้างต้องแคบ |
